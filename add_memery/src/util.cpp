...
namespace imagequeue {
    namespace util {
        ...
        void copyContinuousMat(const cv::Mat &mat, void *buffer) {
            auto matSize = mat.rows * mat.cols * mat.elemSize();
            memcpy(buffer, mat.data, matSize);
        }

        void copyNonContinuousMat(const cv::Mat &mat, void *buffer) {
        ...
        void copyMat(const cv::Mat &mat, void *buffer) {
            assert(buffer);
        ...
        void copyMat(uint32_t rows, uint32_t cols, uint32_t type, void *buffer, cv::Mat &mat) {
