...
namespace imagequeue {
    ...
    public:
         ...
         *
         * */
        void open(std::string name, uint32_t capacity, uint32_t rows, uint32_t cols, uint32_t pixelType, size_t pixelSize);
         ...
         *
         * */
        void open(std::string name, uint32_t capacity, ImageFormat format);

        /**
         * 将当前的 ImageQueue 绑定到图像队列。
         *
         * @param name 队列的全局唯一名称。该参数不能为空串。
         * @param capacity 队列容量。该参数不能为 0。
         ...
         *
         * */
        void open(std::string name, uint32_t capacity, const cv::Mat &prototype);
         ...
         *
         * */
        uint32_t push(const cv::Mat &mat) const;
         ...
         *
         * */
        bool find(uint32_t id, cv::Mat &mat) const;
    };
